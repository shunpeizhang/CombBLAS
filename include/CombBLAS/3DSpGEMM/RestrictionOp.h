...
FullyDistSpVec<IT, ONT> MIS2(SpParMat < IT, INT, DER> A)
{
    ...
    while (cand.getnnz() > 0)
    {
        ...
        //# std::find the smallest random value among a vertex's 1 and 2-hop neighbors
        ...
        //# std::find 1-hop and 2-hop neighbors of new_S_members
...
void RestrictionOp( CCGrid & CMG, SpDCCols<IT, NT> * localmat, SpDCCols<IT, NT> *& R, SpDCCols<IT, NT> *& RT)
{
    if(CMG.layer_grid == 0)
    {
        ...
        FullyDistVec<IT, IT> perm_row(Rop.getcommgrid()); // permutation std::vector defined on layers
        FullyDistVec<IT, IT> perm_col(Rop.getcommgrid()); // permutation std::vector defined on layers
        ...
        R = new SpDCCols<IT,NT>(Rop.seq()); // deep std::copy
        RT = new SpDCCols<IT,NT>(RopT.seq()); // deep std::copy
...
void RestrictionOp( CCGrid & CMG, SpDCCols<IT, NT> * localmat, SpDCCols<IT, NT> *& R, SpDCCols<IT, NT> *& RT)
{
    if(CMG.layer_grid == 0)
    {
        ...
        // ------------ compute MIS-2 ----------------------------
        FullyDistSpVec<IT, IT> mis2 (B.getcommgrid(), B.getncol()); // values of the mis2 std::vector are just "ones"
        mis2 = MIS2<IT>(B);
       	mis2.PrintInfo("MIS original");
        // ------------ Obtain restriction matrix from mis2 ----
        FullyDistVec<IT,IT> ri = mis2.FindInds([](IT x){return true;});

        // std::find the vertices that are not covered by mis2 AND its one hop neighborhood
        FullyDistSpVec<IT,IT> mis2neigh = SpMV<MIS2verifySR<bool, IT>>(B, mis2, false);
        ...
        // std::find the union of mis2neigh and mis2 (ABAB: this function to be wrapped & called "SetUnion")
        mis2neigh = EWiseApply<IT>(mis2neigh, mis2,
                              [](IT x, IT y){return x;},
                              [](IT x, IT y){return true;},
                              true, true, (IT) 1, (IT) 1, true);
        ...
        SpParHelper::Print("R (std::minus M) matrix... ");
        Rop.PrintInfo();
        ...
        // Returns a dense std::vector of nonzero global indices for which the predicate is satisfied on values
        FullyDistVec<IT,IT> extrarows = spones.FindInds([](IT x){return true;});   // dense std::leftovers array is the extra rows
        ...
        FullyDistVec<IT, IT> perm_row(RopFull1.getcommgrid()); // permutation std::vector defined on layers
        FullyDistVec<IT, IT> perm_col(RopFull1.getcommgrid()); // permutation std::vector defined on layers
        ...
        R = new SpDCCols<IT,NT>(RopFull1.seq()); // deep std::copy
        RT = new SpDCCols<IT,NT>(RopT.seq()); // deep std::copy
