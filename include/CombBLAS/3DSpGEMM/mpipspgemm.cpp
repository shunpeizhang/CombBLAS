...
#pragma omp parallel
...

    {
        ...
        else
        {
            ...
            SplitMat(CMG, A, splitA, false);
            SplitMat(CMG, B, splitB, true); //row-split
            if(myrank == 0) std::cout << "RMATs Generated and replicated along layers : time " << MPI_Wtime() - t01 << std::endl;
        ...
        if(myrank == 0) std::cout << "After split: nnzA= " << globalnnzA << " & nnzB= " << globalnnzB;
        ...
        else // default column-threaded
        {
            ...
            if(myrank == 0) std::cout << "\n After multiplication: nnzC= " << nnzC << std::endl << std::endl;
            delete splitC;
