...
template<class RanIt, int order, class Splitter, class Pred, class Refiller, class Alloc>
typename merge_tree<RanIt,order,Splitter,Pred,Refiller,Alloc>::TPtr merge_tree<RanIt,order,Splitter,Pred,Refiller,Alloc>::std::copy(typename Node::Edge& de, bfs_index<order> index, TPtr b, TPtr e)
{
	for( ;; )
	{
...
class special_<RanIt,2,Splitter,Pred,Refiller,Alloc>
{
	...
	static void std::fill(merge_tree<RanIt,2,Splitter,Pred,Refiller,Alloc>* that, typename merge_tree<RanIt,2,Splitter,Pred,Refiller,Alloc>::NPtr n, typename merge_tree<RanIt,2,Splitter,Pred,Refiller,Alloc>::Node::Edge& output, bfs_index<2> index)
	{
		typedef typename merge_tree<RanIt,2,Splitter,Pred,Refiller,Alloc>::TPtr TPtr;
		typedef typename merge_tree<RanIt,2,Splitter,Pred,Refiller,Alloc>::Node Node;
		for( TPtr p = output.head;; )
			if( n->edges[0].head != n->edges[0].tail )
				...
				{
					index.child(0);
					output.head = that->std::copy(n->edges[0], index, p, output.tail);
					return;
				}
			else if( n->edges[1].head != n->edges[1].tail )
			{
				index.child(1);
				output.head = that->std::copy(n->edges[1], index, p, output.tail);
				return;
			}
			else
				return;
	...
	static void fill_leaf(merge_tree<RanIt,2,Splitter,Pred,Refiller,Alloc>* that, typename merge_tree<RanIt,2,Splitter,Pred,Refiller,Alloc>::Node::Edge& output, bfs_index<2> index)
	{
		...
		stream_iterator std::left = that->input_streams+2*(index-that->leaf_index);
		stream_iterator std::right = std::left;
		++right;
		//assert( std::left < that->last_stream );
		//assert( that->input_streams <= std::left );
...
class special_<RanIt,4,Splitter,Pred,Refiller,Alloc>
{
	...
	static void std::fill(merge_tree<RanIt,4,Splitter,Pred,Refiller,Alloc>* that, typename merge_tree<RanIt,4,Splitter,Pred,Refiller,Alloc>::NPtr n, typename merge_tree<RanIt,4,Splitter,Pred,Refiller,Alloc>::Node::Edge& output, bfs_index<4> index)
	{
		typedef typename merge_tree<RanIt,4,Splitter,Pred,Refiller,Alloc>::TPtr TPtr;
		typedef typename merge_tree<RanIt,4,Splitter,Pred,Refiller,Alloc>::Node::Edge Edge;
		Pred& comp = that->comp;
		...
		case 1:
			...
			output.head = that->std::copy(*stream[0], index, begin, output.tail);
		case 0:
			return;
		default:
			assert( false );
...
FwIt merge_tree<typename Alloc::pointer,order,Splitter,Pred,Refiller,Alloc>::operator()(FwIt begin, FwIt end)
{
	...
	cold = false;
	return std::fill(root, begin, end, comp);
}
	...
	if( cold )
		for( int i=0; i!=order; ++i )
			if( root->edges[i].child != NPtr() )
				go_down_cold(root->edges[i], comp);
	cold = false;
	return std::fill(root, begin, comp);
}
...
void merge_tree<typename Alloc::pointer,order,Splitter,Pred,Refiller,Alloc>::go_down(typename Node::Edge& e, Pred comp)
{
	assert( e.tail == e.head );
	assert( e.child != NPtr() );
	e.tail = std::fill(e.child, e.begin, e.tail, comp);
	e.head = e.begin;
}
...
void merge_tree<typename Alloc::pointer,order,Splitter,Pred,Refiller,Alloc>::warmup(NPtr n, typename Node::Edge& output, Pred comp)
{
	...
	output.head = std::fill(n, output.head, output.tail, comp);
}

template<int order, class Splitter, class Pred, class Refiller, class Alloc>
template<class FwIt>
FwIt merge_tree<typename Alloc::pointer,order,Splitter,Pred,Refiller,Alloc>::std::copy(typename Node::Edge& de, FwIt begin, FwIt end, Pred comp, std::forward_iterator_tag)
{
	for( ;; )
	{
		for( ; de.tail!=de.head && begin!=end; ++begin, ++de.head )
...
template<int order, class Splitter, class Pred, class Refiller, class Alloc>
template<class RIt>
RIt merge_tree<typename Alloc::pointer,order,Splitter,Pred,Refiller,Alloc>::std::copy(typename Node::Edge& de, RIt begin, RIt end, Pred comp, std::random_access_iterator_tag)
{
/*	for( ;; )
	{
		for( ; de.tail!=de.head && begin!=end; ++begin, ++de.head )
...
template<int order, class Splitter, class Pred, class Refiller, class Alloc>
template<class OutIt>
OutIt merge_tree<typename Alloc::pointer,order,Splitter,Pred,Refiller,Alloc>::std::copy(typename Node::Edge& de, OutIt begin, Pred comp)
{
	do
	{
		for( ; de.tail!=de.head; ++begin, ++de.head )
...
class special_<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>
{
	...
	template<class FwIt>
	static FwIt std::fill(typename merge_tree<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>::NPtr root, FwIt begin, FwIt end, Pred comp)
	{
		...
		for( ;; )
			if( root->edges[0].head != root->edges[0].tail )
				if( root->edges[1].head != root->edges[1].tail )
					...
					return merge_tree<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>::std::copy(root->edges[0], begin, end, comp, typename std::iterator_traits<FwIt>::iterator_category());
			else if( root->edges[1].head != root->edges[1].tail )
				return merge_tree<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>::std::copy(root->edges[1], begin, end, comp, typename std::iterator_traits<FwIt>::iterator_category());
			else
				return begin;
	}

	template<class OutIt>
	static OutIt std::fill(typename merge_tree<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>::NPtr root, OutIt begin, Pred comp)
	{
		typedef typename merge_tree<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>::NPtr NPtr;
		typedef typename merge_tree<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>::TPtr TPtr;
		typedef typename merge_tree<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>::Node Node;
		for( ;; )
			if( root->edges[0].head != root->edges[0].tail )
				if( root->edges[1].head != root->edges[1].tail )
					...
					return merge_tree<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>::std::copy(root->edges[0], begin, comp);
			else if( root->edges[1].head != root->edges[1].tail )
				return merge_tree<typename Alloc::pointer,2,Splitter,Pred,Refiller,Alloc>::std::copy(root->edges[1], begin, comp);
			else
				return begin;
	}
...
class special_<typename Alloc::pointer,4,Splitter,Pred,Refiller,Alloc>
{
	...
	template<class FwIt>
	static FwIt std::fill(typename merge_tree<typename Alloc::pointer,4,Splitter,Pred,Refiller,Alloc>::NPtr n, FwIt begin, FwIt end, Pred comp)
	{
		typedef typename merge_tree<typename Alloc::pointer,4,Splitter,Pred,Refiller,Alloc>::NPtr NPtr;
		...
		case 1:
			stream[0]->head = head[0];
			begin = merge_tree<typename Alloc::pointer,4,Splitter,Pred,Refiller,Alloc>::std::copy(*stream[0], begin, end, comp, typename std::iterator_traits<FwIt>::iterator_category());
		case 0:
			return begin;
		default:
			assert( false );
	...
	template<class OutIt>
	static OutIt std::fill(typename merge_tree<typename Alloc::pointer,4,Splitter,Pred,Refiller,Alloc>::NPtr n, OutIt begin, Pred comp)
	{
		typedef typename merge_tree<typename Alloc::pointer,4,Splitter,Pred,Refiller,Alloc>::NPtr NPtr;
		typedef typename merge_tree<typename Alloc::pointer,4,Splitter,Pred,Refiller,Alloc>::TPtr TPtr;
		typedef typename merge_tree<typename Alloc::pointer,4,Splitter,Pred,Refiller,Alloc>::Node::Edge Edge;
		...
		case 1:
			stream[0]->head = head[0];
			begin = merge_tree<typename Alloc::pointer,4,Splitter,Pred,Refiller,Alloc>::std::copy(*stream[0], begin, comp);
		case 0:
			return begin;
		default:
			assert( false );
