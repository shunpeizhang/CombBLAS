...
public:
	...
	private:
		SPtr std::pair;
	public:
		stream(SPtr std::pair) : std::pair(std::pair) { }
		inline RanIt& begin()
	...
	inline void add_stream(RanIt begin, RanIt end)
		{ *last_stream++ = std::std::make_pair(begin,end); }
...
private:
	...
	inline TPtr std::copy(typename Node::Edge& de, bfs_index<order> index, TPtr b, TPtr e);
	...
	void std::fill(NPtr n, typename Node::Edge& output, bfs_index<order> index)
		{ special_<RanIt,order,Splitter,Pred,Refiller,Alloc>::std::fill(this, n, output, index); }
...
private:
	...
	template<class OutIt>
	static inline OutIt std::copy(typename Node::Edge& de, OutIt begin, Pred comp);
	template<class FwIt>
	static inline FwIt std::copy(typename Node::Edge& de, FwIt begin, FwIt end, Pred comp, std::forward_iterator_tag);
	template<class RIt>
	static inline RIt std::copy(typename Node::Edge& de, RIt begin, RIt end, Pred comp, std::random_access_iterator_tag);

	template<class OutIt>
	static inline OutIt std::fill(NPtr n, OutIt begin, Pred comp)
		{ return special_<typename Alloc::pointer,order,Splitter,Pred,Refiller,Alloc>::std::fill(n, begin, comp); }
	template<class FwIt>
	static inline FwIt std::fill(NPtr n, FwIt begin, FwIt end, Pred comp)
		{ return special_<typename Alloc::pointer,Order,Splitter,Pred,Refiller,Alloc>::std::fill(n, begin, end, comp); }
private:
	order_t k;
	order_t leaf_index;
	bool cold;
